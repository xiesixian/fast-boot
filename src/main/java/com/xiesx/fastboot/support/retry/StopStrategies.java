...
public final class StopStrategies {
    ...
    /**
     * Returns a stop strategy which never stops retrying. It might be best to try not to abuse services
     * with this kind of behavior when small wait intervals between retry attempts are being used.
     *
    ...
    /**
     * Returns a stop strategy which stops after a given delay. If an unsuccessful attempt is made, this
     * {@link StopStrategy} will check if the amount of time that's passed from the first attempt has
     * exceeded the given delay amount. If it has exceeded this delay, then using this strategy causes
     * the retrying to stop.
     *

...
public class RetryerBuilder<V> {
    ...
    /**
     * Sets the stop strategy used to decide when to stop retrying. The default strategy is to not stop
     * at all .
     *
    ...
    /**
     * Sets the block strategy used to decide how to block between retry attempts. The default strategy
     * is to use Thread#sleep().
     *
    ...
    /**
     * Configures the retryer to retry if a runtime exception (i.e. any <code>RuntimeException</code> or
     * subclass of <code>RuntimeException</code>) is thrown by the call.
     *
     * @return <code>this</code>
     */
    ...
    /**
     * Configures the retryer to retry if an exception satisfying the given predicate is thrown by the
     * call.
     *
     ...
     *
     * @param resultPredicate a predicate applied to the result, and which causes the retryer to retry
    ...
    public Retryer<V> build() {
        AttemptTimeLimiter<V> theAttemptTimeLimiter =
                attemptTimeLimiter == null ? AttemptTimeLimiters.<V>noTimeLimit() : attemptTimeLimiter;
